...
/**
 * 	\defgroup   sentry     Module Sentry
 *  \brief      Sentry module descriptor.
 *
 *  \file       htdocs/sentry/core/modules/modSentry.class.php
 *  \ingroup    sentry
...
class modSentry extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->numero = 104005; // TODO Go on page https://wiki.dolibarr.org/index.php/List_of_modules_id to reserve an id number for your module
		...
		// Module label (no space allowed), used if translation string 'ModuleSentryName' not found (Sentry is name of module).
		$this->name = preg_replace('/^mod/i', '', get_class($this));

		// DESCRIPTION_FLAG
		// Module description, used if translation string 'ModuleSentryDesc' not found (Sentry is name of module).
		$this->description = "SentryDescription";
		// Used only if file README.md and README-LL.md not found.
		$this->descriptionlong = "SentryDescription";

		// Author
		$this->editor_name = 'ATM Consulting';
		$this->editor_url = 'www.atm-consulting.fr';		// Must be an external online web site
		...
		// Key used in llx_const table to save module status enabled/disabled (where SENTRY is value of property name of module in uppercase)
		...
		// $this->tabs[] = array('data' => 'objecttype:+tabname1:Title1:mylangfile@sentry:$user->hasRight(\'sentry\', \'read\'):/sentry/mynewtab1.php?id=__ID__');
		// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
		// $this->tabs[] = array('data' => 'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@sentry:$user->hasRight(\'othermodule\', \'read\'):/sentry/mynewtab2.php?id=__ID__',
		...
		/* Example:
		 ...
		 'tabcond' => array(isModEnabled('sentry'), isModEnabled('sentry'), isModEnabled('sentry')),
		...
		$this->cronjobs = array(
			...
			//      'label' => 'MyJob label',
			//      'jobtype' => 'method',
			//      'class' => '/sentry/class/myobject.class.php',
			//      'objectname' => 'MyObject',
		...
		//    1=>array('label'=>'My label', 'jobtype'=>'command', 'command'=>'', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>1, 'unitfrequency'=>3600*24, 'status'=>0, 'test'=>'isModEnabled("sentry")', 'priority'=>50)
		// );
		...
		$this->rights[$r][1] = 'Read objects of Sentry'; // Permission label
		$this->rights[$r][4] = 'myobject';
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->hasRight('sentry', 'myobject', 'read'))
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", ($o * 10) + 2); // Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update objects of Sentry'; // Permission label
		$this->rights[$r][4] = 'myobject';
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->hasRight('sentry', 'myobject', 'write'))
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", ($o * 10) + 3); // Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete objects of Sentry'; // Permission label
		$this->rights[$r][4] = 'myobject';
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->hasRight('sentry', 'myobject', 'delete'))
		$r++;
		*/
		...
		/*
		$this->menu[$r++]=array(
			'fk_menu' => 'fk_mainmenu=sentry',      // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			...
			'position' => 1000 + $r,
			'enabled' => 'isModEnabled("sentry")', // Define condition to show or hide menu entry. Use 'isModEnabled("sentry")' if entry must be visible if module is enabled.
			'perms' => '$user->hasRight("sentry", "myobject", "read")',
			'target' => '',
			'user' => 2,				                // 0=Menu for internal users, 1=external users, 2=both
			'object' => 'MyObject'
		);
		$this->menu[$r++]=array(
			'fk_menu' => 'fk_mainmenu=sentry,fk_leftmenu=myobject',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'type' => 'left',			                // This is a Left menu entry
			'titre' => 'New_MyObject',
			'mainmenu' => 'sentry',
			'leftmenu' => 'sentry_myobject_new',
			'url' => '/sentry/myobject_card.php?action=create',
			'langs' => 'sentry@sentry',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position' => 1000 + $r,
			'enabled' => 'isModEnabled("sentry")', // Define condition to show or hide menu entry. Use 'isModEnabled("sentry")' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms' => '$user->hasRight("sentry", "myobject", "write")'
			'target' => '',
			'user' => 2,				                // 0=Menu for internal users, 1=external users, 2=both
			'object' => 'MyObject'
		);
		$this->menu[$r++]=array(
			...
			'titre' => 'List_MyObject',
			'mainmenu' => 'sentry',
			'leftmenu' => 'sentry_myobject_list',
			'url' => '/sentry/myobject_list.php',
			'langs' => 'sentry@sentry',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position' => 1000 + $r,
			'enabled' => 'isModEnabled("sentry")', // Define condition to show or hide menu entry. Use 'isModEnabled("sentry")' if entry must be visible if module is enabled.
			'perms' => '$user->hasRight("sentry", "myobject", "read")'
		...
		*/
		...
		//$keyforclass = 'MyObjectLine'; $keyforclassfile='/sentry/class/myobject.class.php'; $keyforelement='myobjectline@sentry'; $keyforalias='tl';
		//include DOL_DOCUMENT_ROOT.'/core/commonfieldsinexport.inc.php';
		$keyforselect='myobject'; $keyforaliasextra='extra'; $keyforelement='myobject@sentry';
		include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
		//$keyforselect='myobjectline'; $keyforaliasextra='extraline'; $keyforelement='myobjectline@sentry';
		//include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
		...
		$this->export_sql_end[$r]  =' FROM '.$this->db->prefix().'sentry_myobject as t';
		//$this->export_sql_end[$r]  .=' LEFT JOIN '.$this->db->prefix().'sentry_myobject_line as tl ON tl.fk_myobject = t.rowid';
		...
		$this->import_convertvalue_array[$r] = array(
			't.ref' => array(
				'rule'=>'getrefifauto',
				'class'=>(!getDolGlobalString('SENTRY_MYOBJECT_ADDON') ? 'mod_myobject_standard' : getDolGlobalString('SENTRY_MYOBJECT_ADDON')),
				'path'=>"/core/modules/sentry/".(!getDolGlobalString('SENTRY_MYOBJECT_ADDON') ? 'mod_myobject_standard' : getDolGlobalString('SENTRY_MYOBJECT_ADDON')).'.php',
				'classobject'=>'MyObject',
				'pathobject'=>'/sentry/class/myobject.class.php',
			),
	...
	public function init($options = '')
	{
		...
		//$result = $this->_load_tables('/install/mysql/', 'sentry');
		$result = $this->_load_tables('/sentry/sql/');
		if ($result < 0) {
